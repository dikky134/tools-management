...
const MONTHLY_DATA = [
  { month: 'Mar', borrowings: 12, returns: 11, damage: 1 },
  { month: 'Apr', borrowings: 18, returns: 17, damage: 2 },
  { month: 'May', borrowings: 15, returns: 14, damage: 1 },
  { month: 'Jun', borrowings: 22, returns: 20, damage: 3 },
  { month: 'Jul', borrowings: 19, returns: 19, damage: 0 },
  { month: 'Aug', borrowings: 28, returns: 25, damage: 2 },
  { month: 'Sep', borrowings: 14, returns: 12, damage: 2 },
];

const chartTooltipStyle = { backgroundColor: '#18181b', border: '1px solid #3f3f46', borderRadius: 2, fontSize: 12 };

export default function AdminDash() {
  const { tools, users, borrowings, maintenanceRequests, notifications, activityLogs } = useApp();
  ...
  const stats = {
    ...
    mechAvail: users.filter(u => u.role === 'MECHANIC' && u.mechanicStatus === 'AVAILABLE').length,
    mechBusy: users.filter(u => u.role === 'MECHANIC' && u.mechanicStatus === 'BUSY').length,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { label: '+ Add Tool', to: '/tools?action=add', color: 'bg-amber-500 hover:bg-amber-400 text-zinc-900' },
          { label: 'Scan Tool', to: '/scan', color: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200' },
          { label: 'Maintenance', to: '/maintenance', color: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200' },
          { label: 'Borrowings', to: '/borrowings', color: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200' },
          { label: 'Reports', to: '/reports', color: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200' },
        ].map(a => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label="Employees" value={stats.employees} color="#a78bfa" />
        <StatCard label="Avail. Mechanics" value={`${stats.mechAvail}/${stats.mechanics}`} color="#4ade80" sub="mechanics available" />
        <StatCard label="Active Borrowings" value={stats.activeBorrowings} color="#60a5fa" />
        <StatCard label="Pending Maint." value={stats.pendingMaintenance} color="#fbbf24" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-sm">
          ...
          <div className="divide-y divide-zinc-800/50">
            ...
            {stats.overdue === 0 && stats.pendingMaintenance === 0 && stats.damaged === 0 && (
              <div className="px-4 py-6 text-center text-sm text-zinc-600">No active alerts</div>
            )}
          </div>
        </div>
